...
export async function generateMetadata({ params }) {
  ...
  const data = await getSinglePostData(slug, "wp-json/wp/v2/news");
  const post = data?.[0];
  ...
  return {
    ...
    description:
      seoData?.description || "Latest school news, announcements, and updates.",
    metadataBase: new URL(process.env.siteUrl),
    openGraph: {
      title: seoData?.title || getPostTitle(post),
      description:
        seoData?.description || "Latest school news, announcements, and updates.",
      url: `${process.env.siteUrl}/news/${slug}`,
...
export default async function NewsSinglePage({ params }) {
  ...
  const [data, options] = await Promise.all([
    getSinglePostData(slug, "wp-json/wp/v2/news"),
    getOptions(),
  ]);
  ...
  return (
    <>
      ...
      <main className="mt-24">
        <PostDetailPage post={post} contentLabel="News" />
      </main>
      <Footer
        showFooterCta={true}
